...
static int mangle_replace (char arr[RP_PASSWORD_SIZE], int arr_len, char oldc, char newc)
{
#ifdef HASHDOG_USE_SSE2
  int arr_pos = 0;
  const __m128i v_old = _mm_set1_epi8 (oldc);
  ...
  for (; arr_pos + 16 <= arr_len; arr_pos += 16)
  {
    ...
    __m128i mask = _mm_cmpeq_epi8 (data, v_old);
    // result = (mask & v_new) | (~mask & data)
    __m128i result = _mm_or_si128 (_mm_and_si128 (mask, v_new),
                                    _mm_andnot_si128 (mask, data));
    _mm_storeu_si128 ((__m128i *) (arr + arr_pos), result);
  }
  for (; arr_pos < arr_len; arr_pos++)
  {
    if (arr[arr_pos] == oldc) arr[arr_pos] = newc;
  }
#else
...
static int mangle_replace_class_l (char arr[RP_PASSWORD_SIZE], int arr_len, char newc)
{
  ...
  for (; arr_pos + 16 <= arr_len; arr_pos += 16)
  {
    ...
    __m128i is_lower = _mm_and_si128 (_mm_cmpgt_epi8 (data, v_a_minus_1),
                                       _mm_cmpgt_epi8 (v_z_plus_1, data));
    __m128i result = _mm_or_si128 (_mm_and_si128 (is_lower, v_new),
                                    _mm_andnot_si128 (is_lower, data));
    _mm_storeu_si128 ((__m128i *) (arr + arr_pos), result);
  }
  for (; arr_pos < arr_len; arr_pos++)
  {
    if (class_lower (arr[arr_pos])) arr[arr_pos] = newc;
  }
#else
  for (int arr_pos = 0; arr_pos < arr_len; arr_pos++)
  {
...
static int mangle_replace_class_u (char arr[RP_PASSWORD_SIZE], int arr_len, char newc)
{
  ...
  for (; arr_pos + 16 <= arr_len; arr_pos += 16)
  {
    ...
    __m128i is_upper = _mm_and_si128 (_mm_cmpgt_epi8 (data, v_A_minus_1),
                                       _mm_cmpgt_epi8 (v_Z_plus_1, data));
    __m128i result = _mm_or_si128 (_mm_and_si128 (is_upper, v_new),
                                    _mm_andnot_si128 (is_upper, data));
    _mm_storeu_si128 ((__m128i *) (arr + arr_pos), result);
  }
  for (; arr_pos < arr_len; arr_pos++)
  {
    if (class_upper (arr[arr_pos])) arr[arr_pos] = newc;
  }
#else
  for (int arr_pos = 0; arr_pos < arr_len; arr_pos++)
  {
...
static int mangle_replace_class_d (char arr[RP_PASSWORD_SIZE], int arr_len, char newc)
{
  ...
  for (; arr_pos + 16 <= arr_len; arr_pos += 16)
  {
    ...
    __m128i result = _mm_or_si128 (_mm_and_si128 (is_digit, v_new),
                                    _mm_andnot_si128 (is_digit, data));
    _mm_storeu_si128 ((__m128i *) (arr + arr_pos), result);
  }
  for (; arr_pos < arr_len; arr_pos++)
  {
    if (class_num (arr[arr_pos])) arr[arr_pos] = newc;
  }
#else
  for (int arr_pos = 0; arr_pos < arr_len; arr_pos++)
  {
